# =============================================================================
# Tasks C & D: Parameter Estimation and Inverse Modeling Framework
# =============================================================================
#
# Configure the PEtab model for estimating parameters across multiple experiments.
# Specify experimental conditions, observable outputs, and unknown parameters.
# Perform multi-start optimization and evaluate estimated vs actual parameters.
# =============================================================================
...
Initialize the PEtab model for parameter estimation.

Arguments:
- `rn`: Fully constructed Catalyst.jl ReactionSystem
- `measurements_df`: DataFrame formatted according to PEtab requirements

Returns: Configured PEtabODEProblem for optimization
"""
function setup_petab_model(rn, measurements_df)
    @unpack M, C, F, Temp, G1, G2 = rn

    # --- Observables ---
    # Measured quantity: total remaining solid mass = M + C + F
    # Measurement noise assumed Gaussian with constant variance (σ = 0.005)
    ...
    # Each experiment is assigned its own heating rate (β) and oxygen level (PO₂)
    # These values override the base model parameters per condition
    @unpack PO2, beta = rn
    ...
    # Following assignment guidance: log10(A) is used instead of A for stability
    # Total estimated parameters: 11 kinetic + 1 noise parameter
    petab_parameters = [
        # Matrix decomposition parameters
        ...
        # Measurement noise parameter (estimated together with kinetics)
        PEtabParameter(:sigma_mass, value=0.005, lb=1e-4, ub=0.1,    scale=:log10),
    ]

    # --- Initial Conditions (identical for all experiments) ---
    state_map = [
        M    => INITIAL_M,
        C    => INITIAL_C,
        F    => INITIAL_F,
    ...
    @unpack nu_char = rn
    parameter_map = [
        nu_char => 0.2,  # Char yield is treated as a fixed constant
    ]

    # --- Construct PEtab Model ---
    model = PEtabModel(
        rn,
        observables,
        measurements_df,
...
Execute multi-start optimization to estimate kinetic parameters.
Instead of random Latin Hypercube sampling, controlled perturbations
around nominal values are used to prevent numerical instability in
Arrhenius-type expressions (which can easily overflow).

Arguments:
- `petab_prob`: Configured PEtabODEProblem
- `n_multistarts`: Number of initial guesses (1 nominal + perturbed starts)

Returns: Result structure containing optimal parameters and statistics
"""
function run_parameter_estimation(petab_prob; n_multistarts=10)
    ...
    # Generate initial guesses: nominal values plus bounded perturbations
    # Avoids unstable parameter combinations from random sampling
    Random.seed!(123)
    ...
    for i in 1:n_multistarts
        if i == 1
            ...
            # Apply ±20% perturbation within parameter bounds
    ...
    # Return structured output compatible with later analysis
...
Generate a table comparing true parameter values with estimated ones.

Returns: DataFrame summarizing parameter accuracy
"""
...
Display the parameter comparison table in a formatted layout.
"""
